...
if ($resultCheck > 0) {
    ...
    // to cancel dislike 
    if ($down == $Oridown) {
        $query = "UPDATE likepost SET t_down=0, t_up=0 WHERE user_id=$userid AND idea_id=$id";
        $query_run = mysqli_query($conn, $query);
    } else {
        $query = "UPDATE likepost SET t_down=1, t_up=0 WHERE user_id=$userid AND idea_id=$id";
        $query_run = mysqli_query($conn, $query);
    }
} else {
